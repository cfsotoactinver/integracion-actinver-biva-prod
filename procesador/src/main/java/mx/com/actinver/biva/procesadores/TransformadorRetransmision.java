...
public class TransformadorRetransmision implements ReglasDeNegocio {
	...
	private final static Logger LOGGER = LoggerFactory
			.getLogger(TransformadorRetransmision.class);
	...
	private final static String INSERT_ITCH_UHECHO = "INSERT INTO PRODSQL.ITCH_UHECHO_RH (ORDERBOOK,FOLIO_ASIGNACION,CANTIDAD,PRECIO,TRADE_INDICATOR,HORAORIGEN) VALUES (";
	...
	private String obtenerSerie(String secCode) {
		...
		if (secCode != null && !secCode.isEmpty()) {
			try {
				respuesta = secCode.substring(secCode.indexOf(ESPACIO) + 1,
						secCode.length());
			} catch (IndexOutOfBoundsException indexOutOfBoundsException) {
	...
	public Boolean procesaMensajeA(A mensajeA) throws Exception {
		...
		if (orden.getNumeroDeOrden() == null) {
			Emisora emisora = memoria
					.obtenerNotificacionDeEmisoraPorCartera(mensajeA
							.getOrderBook());
			...
			if (cartera.getNumeroDeDecimales() != null) {
				...
				if (mensajeA.getOrderNumber() == 0
						&& mensajeA.getQuantity() == 0) {
					...
					String idPostura = orden.getEmisora()
							+ orden.getCarteraDePedidos()
							+ orden.getVerboDeLaOrden();
					Integer posicion = 0;
					if (orden.getVerboDeLaOrden() != null) {
						if (orden.getVerboDeLaOrden().equals("B")) {
							Compra compra = memoria.buscarArregloCompra(orden
									.getEmisora(), orden.getCarteraDePedidos()
									.toString());
							compra.setEmisora(orden.getEmisora());
							compra.setIdcompra(idPostura);
							if (compra.getNiveles() != null) {
								niveles = compra.getNiveles();
								Nivel precioABuscar = niveles
										.stream()
										.filter(order -> order.getPrecio()
												.equals(orden.getPrecio()))
										.findAny().orElse(null);
								posicion = niveles.indexOf(precioABuscar);
								if (posicion >= 0) {
									nivel = niveles.get(posicion);
									nivel.setPrecio(orden.getPrecio());
									nivel.setVolumen(nivel.getVolumen()
											+ orden.getCantidad());
									nivel.setNumeroDeOrdenes(nivel
											.getNumeroDeOrdenes() + 1);
									niveles.remove(niveles
											.indexOf(precioABuscar));
							...
							Venta venta = memoria.buscarArregloVenta(orden
									.getEmisora(), orden.getCarteraDePedidos()
									.toString());
	...
	public Boolean procesaMensajeD(D mensajeD) throws Exception {
		...
		if (orden != null) {
			Emisora emisora = memoria
					.obtenerNotificacionDeEmisoraPorCartera(orden
							.getCarteraDePedidos());
			Cartera cartera = this.memoria.buscarCartera(orden
					.getCarteraDePedidos());
			List<Nivel> niveles = new ArrayList<Nivel>();
			Nivel nivel = new Nivel();
			String idPostura = orden.getEmisora() + orden.getCarteraDePedidos()
					+ orden.getVerboDeLaOrden();
			Integer posicion = 0;
			if (orden.getVerboDeLaOrden() != null
					&& orden.getEliminaOrden().equals("N")) {
				if (orden.getVerboDeLaOrden().equals("B")) {
					Compra compra = memoria.buscarArregloCompra(orden
							.getEmisora(), orden.getCarteraDePedidos()
							.toString());
					if (compra.getNiveles() != null) {
						...
						Nivel precioABuscar = niveles
								.stream()
								.filter(order -> order.getPrecio().equals(
										orden.getPrecio())).findAny()
								.orElse(null);
						posicion = niveles.indexOf(precioABuscar);
					...
					Venta venta = memoria.buscarArregloVenta(
							orden.getEmisora(), orden.getCarteraDePedidos()
									.toString());
					if (venta.getNiveles() != null) {
						niveles = venta.getNiveles();
						Nivel precioABuscar = niveles
						...
						if (posicion >= 0) {
							...
							if (nivel.getNumeroDeOrdenes() >= 2) {
								...
								niveles.remove(niveles.indexOf(precioABuscar));
								niveles.add(posicion, nivel);
								niveles.sort(Comparator
										.comparing(Nivel::getPrecio));
							} else {
								niveles.remove(niveles.indexOf(precioABuscar));
								niveles.sort(Comparator
										.comparing(Nivel::getPrecio));
	...
	public Boolean procesaMensajeU(U mensajeU) throws Exception {
		...
		if (orden != null) {
			Cartera cartera = memoria
					.buscarCartera(orden.getCarteraDePedidos());
			if (cartera.getNumeroDeDecimales() != null
					&& orden.getEliminaOrden().equals("N")) {
				Emisora emisora = memoria
						.obtenerNotificacionDeEmisoraPorCartera(orden
				...
				Integer decimales = cartera.getNumeroDeDecimales();
				memoria.eliminaOrdenU(mensajeU.getOriginalOrderNumber());
				ordenNueva.setCarteraDePedidos(Integer.valueOf(cartera
						.getCarteraDePedido()));
				...
				ordenNueva.setPrecio(Double.valueOf(getDecimal(
						mensajeU.getPrice(), decimales)));
					...
					if (orden.getVerboDeLaOrden().equals("B")) {
						...
						if (compra.getNiveles() != null) {
							...
							if (posicion >= 0) {
								...
								nivel = new Nivel();
								precioABuscar = niveles
										.stream()
										.filter(order -> order.getPrecio()
												.equals(ordenNueva.getPrecio()))
										.findAny().orElse(null);
								posicion = niveles.indexOf(precioABuscar);
								if (posicion >= 0) {
									nivel = niveles.get(posicion);
		...
		if (orden != null) {
			...
			if (orden.getVerboDeLaOrden() != null
				...
				Emisora emisora = memoria
						.obtenerNotificacionDeEmisoraPorCartera(orden
								.getCarteraDePedidos());
				Cartera cartera = memoria.buscarCartera(orden
						.getCarteraDePedidos());
				if (orden.getVerboDeLaOrden().equals("B")) {
					Compra compra = memoria.buscarArregloCompra(orden
							.getEmisora(), orden.getCarteraDePedidos()
					...
					if (compra.getNiveles() != null) {
						...
						if (posicion >= 0) {
							if (orden.getCantidad() == mensajeE
									.getExecutedQuantity()) {
								if (niveles.get(niveles.indexOf(precioABuscar))
										.getNumeroDeOrdenes() == 1) {
									niveles.remove(niveles
											.indexOf(precioABuscar));
									memoria.eliminaOrdenE(mensajeE
											.getOrderNumber());
								} else {
									nivel = niveles.get(niveles
											.indexOf(precioABuscar));
									nivel.setPrecio(orden.getPrecio());
									nivel.setVolumen(nivel.getVolumen()
											- mensajeE.getExecutedQuantity());
									nivel.setNumeroDeOrdenes(nivel
											.getNumeroDeOrdenes() - 1);
									niveles.remove(niveles
											.indexOf(precioABuscar));
							...
							} else if (orden.getCantidad() > mensajeE
									.getExecutedQuantity()) {
								nivel = niveles.get(posicion);
								nivel.setPrecio(orden.getPrecio());
								nivel.setVolumen(nivel.getVolumen()
								...
								niveles.remove(niveles.indexOf(precioABuscar));
								niveles.add(posicion, nivel);
								orden.setCantidad(orden.getCantidad()
										- mensajeE.getExecutedQuantity());
								memoria.actualizarOrden(orden);
							...
							emisora.setInstrumentNumber(orden
									.getCarteraDePedidos());
							emisora.setOpeningPrice(cartera
									.getPrecioDeApertura());
							emisora.setPostura("compra");
							if (compra.getNiveles().size() > 0) {
								emisora.setBidVolume((compra.getNiveles()
										.get(0).getVolumen()));
								emisora.setBestBid(compra.getNiveles().get(0)
										.getPrecio());
							...
							emisora.setPreviousSettlementPrice(emisora
									.getLastPrice());
							emisora.setLastPrice(orden.getPrecio());
							emisora.setLastVolume(mensajeE
									.getExecutedQuantity());
							emisora.setLastTime(mensajeE.getFechaHoraEmision());
							if (emisora.getMaxPriceOfTheDay() != null) {
								if (emisora.getMaxPriceOfTheDay() < orden
										.getPrecio()) {
									emisora.setMaxPriceOfTheDay(orden
											.getPrecio());
									emisora.setMaxTime(mensajeE
											.getFechaHoraEmision());
							...
							if (emisora.getLowPriceOfTheDay() != null) {
								if (emisora.getLowPriceOfTheDay() > orden
										.getPrecio()) {
									emisora.setLowPriceOfTheDay(orden
											.getPrecio());
									emisora.setLowTime(mensajeE
											.getFechaHoraEmision());
							...
							if (emisora.getVolumenAcumulado() != null) {
								emisora.setVolumenAcumulado(emisora
										.getVolumenAcumulado()
										+ mensajeE.getExecutedQuantity());
							} else {
								emisora.setVolumenAcumulado(mensajeE
										.getExecutedQuantity());
							}
							if (emisora.getPrecioAcumulado() != null
									&& emisora.getPrecioAcumulado() > 0) {

								Double monto = (orden.getPrecio() * mensajeE
										.getExecutedQuantity());
								emisora.setPrecioAcumulado(emisora
										.getPrecioAcumulado()
										+ Double.valueOf(getDecimalPoint(
												monto.toString(),
												cartera.getNumeroDeDecimales())));

							} else {
								Double monto = (orden.getPrecio() * mensajeE
										.getExecutedQuantity());
								emisora.setPrecioAcumulado(Double
										.valueOf(getDecimalPoint(
												monto.toString(),
												cartera.getNumeroDeDecimales())));

							}
							Double average = emisora.getPrecioAcumulado()
									/ emisora.getVolumenAcumulado();
							emisora.setAveragePriceDay(getDecimalPoint(
									average.toString(),
									cartera.getNumeroDeDecimales()));
							if (emisora.getTradedAmount() != null) {
								emisora.setTradedAmount(emisora
										.getTradedAmount()
										+ (orden.getPrecio() * mensajeE
												.getExecutedQuantity()));
							} else {
								emisora.setTradedAmount((orden.getPrecio() * mensajeE
										.getExecutedQuantity()));
							}
							emisora.setTradedAmount(emisora
									.getPrecioAcumulado());
							...
							emisora.setTradedVolume(emisora
									.getVolumenAcumulado());
							memoria.actualizaEmisora(emisora);
							Hecho hecho = memoria
									.obtenerNotificacionDeHechoPorCartera(orden
											.getCarteraDePedidos());
							hecho.setNumeroDeInstrumento(cartera
									.getCarteraDePedido());
							...
							hecho.setFolioDelHecho(mensajeE.getMatchNumber()
									.toString());
							Character tipoDeConcertacion = mensajeE
									.getTradeIndicator();
							hecho.setHoraDelHecho(mensajeE
									.getFechaHoraEmision());
							...
							hecho.setTipoDeConcertacion(tipoDeConcertacion
									.toString());

							hecho.setFijaPrecio("1");
							hecho.setTipoDeOperacion("C");
							Double importe = mensajeE.getExecutedQuantity()
									* orden.getPrecio();
							...
							Profundidad profundidad = memoria
									.obtenerNotificacionDeProfundiodadPorCartera(orden
											.getCarteraDePedidos());
							int i = 0;
							...
							profundidad.setNumeroDeInstrumento(orden
									.getCarteraDePedidos());
						...
						if (posicion >= 0) {
							...
							if (venta.getNiveles().size() > 0) {
								emisora.setOfferVolume((venta.getNiveles().get(
										0).getVolumen()));
								emisora.setBestOffer(venta.getNiveles().get(0)
										.getPrecio());
							} else {
							...
							if (emisora.getTradedAmount() != null) {
								...
								Double monto = (orden.getPrecio() * mensajeE
										.getExecutedQuantity());
								emisora.setTradedAmount(Double
										.valueOf(getDecimalPoint(
												monto.toString(),
												cartera.getNumeroDeDecimales())));
							}
							...
							Hecho hecho = memoria
									.obtenerNotificacionDeHechoPorCartera(orden
											.getCarteraDePedidos());
							hecho.setNumeroDeInstrumento(orden
									.getCarteraDePedidos().toString());
							...
							profundidad.setNiveles(nprof.size());
							profundidad.setFechaActualizacion(format
									.format(sysdate));
							memoria.actualizaProfundidad(profundidad);
							feedUnico.sendBody(emisora);
							feedUnico.sendBody(profundidad);
			...
			if (orden.getVerboDeLaOrden() != null
				...
				if (orden.getVerboDeLaOrden().equals("B")) {
					...
					if (compra.getNiveles() != null) {
						...
						Integer posicion = niveles.indexOf(precioABuscar);
						if (posicion >= 0) {
							if (orden.getCantidad() == mensajeC
									.getExecutedQuantity()) {
								if (niveles.get(niveles.indexOf(precioABuscar))
										.getNumeroDeOrdenes() == 1) {
									niveles.remove(niveles
											.indexOf(precioABuscar));
									memoria.eliminaOrdenE(mensajeC
											.getOrderNumber());
							...
							} else if (orden.getCantidad() > mensajeC
									.getExecutedQuantity()) {
								nivel = niveles.get(posicion);
								nivel.setPrecio(orden.getPrecio());
								nivel.setVolumen(nivel.getVolumen()
							...
							if (mensajeC.getPrintable() == 'Y') {
								...
								emisora.setLastVolume(mensajeC
										.getExecutedQuantity());
								emisora.setLastTime(mensajeC
										.getFechaHoraEmision());
								if (emisora.getVolumenAcumulado() != null) {
									emisora.setVolumenAcumulado(emisora
											.getVolumenAcumulado()
									...
									emisora.setVolumenAcumulado(mensajeC
											.getExecutedQuantity());
								}
								if (emisora.getPrecioAcumulado() != null
										&& emisora.getPrecioAcumulado() > 0) {

									Long monto = (mensajeC.getExecutionPrice() * mensajeC
											.getExecutedQuantity());
									emisora.setPrecioAcumulado(emisora
											.getPrecioAcumulado()
											+ Double.valueOf(getDecimalPoint(
									...
									Long monto = (mensajeC.getExecutionPrice() * mensajeC
											.getExecutedQuantity());
									emisora.setPrecioAcumulado(Double
											.valueOf(getDecimalPoint(monto
													.toString(), cartera
													.getNumeroDeDecimales())));
							...
							emisora.setPreviousSettlementPrice(emisora
									.getLastPrice());
							emisora.setLastPriceOfTheDay(Double
									.valueOf(getDecimal(
											mensajeC.getExecutionPrice(),
											cartera.getNumeroDeDecimales())));
							...
							hecho.setFolioDelHecho(mensajeC.getMatchNumber()
									.toString());

							Character tipoDeConcertacion = mensajeC
									.getTradeIndicator();
							if (tipoDeConcertacion.equals('U')) {
								tipoDeConcertacion = 'C';
							} else if (tipoDeConcertacion.equals('R')) {
							...
							hecho.setFijaPrecio("1");
							hecho.setTipoDeOperacion("C");
							Double importe = mensajeC.getExecutedQuantity()
									* precio;
							hecho.setImporte(importe.toString());
							hecho.setCompra("BIVA");
							hecho.setVende("BIVA");
							hecho.setLiquidacion(4);
							hecho.setIndicadorDeSubasta("     ");
							hecho.setHoraDelHecho(mensajeC
									.getFechaHoraEmision());
							memoria.actualizaHecho(hecho);
							Profundidad profundidad = memoria
									.obtenerNotificacionDeProfundiodadPorCartera(orden
	...
	public Boolean procesaMensajeP(P mensajeP) throws Exception {
		if (memoria.buscarCartera(mensajeP.getOrderBook())
				.getNumeroDeDecimales() != null) {
			Hecho hecho = memoria.obtenerNotificacionDeHechoPorCartera(mensajeP
					.getOrderBook());
			Emisora emisora = memoria
					.obtenerNotificacionDeEmisoraPorCartera(mensajeP
							.getOrderBook());
			Cartera cartera = memoria.buscarCartera(mensajeP.getOrderBook());
			Double precio = Double.valueOf(getDecimal(
					mensajeP.getExecutionPrice(),
					cartera.getNumeroDeDecimales()));
			Character tipoDeConcertacion = mensajeP.getTradeIndicator();
			emisora.setIssuer(cartera.getEmisora());
			emisora.setSeries(cartera.getSerie());
			emisora.setIsin(cartera.getIsin());
			emisora.setInstrumentNumber(Integer.valueOf(cartera
					.getCarteraDePedido()));
			...
			if (mensajeP.getPrintable() == 'Y') {
				...
				if (emisora.getVolumenAcumulado() != null) {
					emisora.setVolumenAcumulado(emisora.getVolumenAcumulado()
							+ mensajeP.getExecutedQuantity());
				} else {
				...
				if (emisora.getPrecioAcumulado() != null
					...
					Long monto = (mensajeP.getExecutionPrice() * mensajeP
							.getExecutedQuantity());
					emisora.setPrecioAcumulado(emisora.getPrecioAcumulado()
							+ Double.valueOf(getDecimalPoint(monto.toString(),
									cartera.getNumeroDeDecimales())));
				...
				Double average = emisora.getPrecioAcumulado()
						/ emisora.getVolumenAcumulado();
				emisora.setAveragePriceDay(getDecimalPoint(average.toString(),
						cartera.getNumeroDeDecimales()));
			}
			...
			if (emisora.getTradedAmount() != null) {
				Long monto = (mensajeP.getExecutionPrice() * mensajeP
						.getExecutedQuantity());
				emisora.setTradedAmount(emisora.getTradedAmount()
						+ Double.valueOf(getDecimalPoint(monto.toString(),
								cartera.getNumeroDeDecimales())));
			} else {
				Long monto = (mensajeP.getExecutionPrice() * mensajeP
	...
	public Boolean procesaMensajeR(R mensajeR) throws Exception {
		...
		Hecho hecho = memoria.obtenerNotificacionDeHechoPorCartera(mensajeR
				.getOrderBook());
		Emisora emisora = memoria
				.obtenerNotificacionDeEmisoraPorCartera(mensajeR.getOrderBook());
		Profundidad profundidad = memoria
				.obtenerNotificacionDeProfundiodadPorCartera(mensajeR
						.getOrderBook());
	...
	public Boolean procesaMensajeH(H mensajeH) throws Exception {
		Emisora emisora = memoria
				.obtenerNotificacionDeEmisoraPorCartera(mensajeH.getOrderBook());
		...
		memoria.actualizaEmisora(emisora);
		Profundidad profundidad = memoria
				.obtenerNotificacionDeProfundiodadPorCartera(mensajeH
						.getOrderBook());
		if (String.valueOf(status).equals("T")) {
			profundidad.setInstrumentStatus(String.valueOf("N"));
		} else {
	 ...
	 * 
	 * @see
	 * mx.com.actinver.biva.persistencia.PersistenciaBaseDeDatos#registraEnUhecho
	 * (mx .com.biva.model.E, mx.com.biva.model.C, mx.com.biva.model.P)
	 */
	public String registraEnUhecho(E mensajeE, C mensajeC, P mensajeP)
			throws Exception {
		...
		if (mensajeE == null && mensajeC == null && mensajeP == null) {
			...
			if (mensajeE != null) {
				...
				if (ord.getCarteraDePedidos() != null) {
					miString.append("'" + ord.getCarteraDePedidos() + "'")
							.append(",");
				} else {
					miString.append("0").append(",");
				}
				if (mensajeE.getMatchNumber() != null) {
					miString.append("'" + mensajeE.getMatchNumber() + "'")
							.append(",");
				} else {
					miString.append("' '").append(",");
				}
				if (mensajeE.getExecutedQuantity() != null) {
					miString.append("'" + mensajeE.getExecutedQuantity() + "'")
							.append(",");
				} else {
					miString.append("0").append(",");
				}
				...
				if (mensajeE.getTradeIndicator() != null) {
					miString.append("'" + mensajeE.getTradeIndicator() + "'")
							.append(",");
				} else {
					miString.append("' '").append(",");
				}
			...
			if (mensajeC != null) {
				...
				if (mensajeC.getMatchNumber() != null) {
					miString.append("'" + mensajeC.getMatchNumber() + "'")
							.append(",");
				} else {
					miString.append("' '").append(",");
				}
				if (mensajeC.getExecutedQuantity() != null) {
					miString.append("'" + mensajeC.getExecutedQuantity() + "'")
							.append(",");
				} else {
					miString.append("0").append(",");
				}
				if (mensajeC.getExecutionPrice() != null) {
					miString.append("'" + mensajeC.getExecutionPrice() + "'")
							.append(",");
				} else {
					miString.append("'0.0'").append(",");
				}
				if (mensajeC.getTradeIndicator() != null) {
					miString.append("'" + mensajeC.getTradeIndicator() + "'")
							.append(",");
				} else {
					miString.append("' '").append(",");
				}
			...
			if (mensajeP != null) {
				...
				if (mensajeP.getOrderBook() != null) {
					miString.append("'" + mensajeP.getOrderBook() + "'")
							.append(",");
				} else {
					miString.append("0").append(",");
				}
				if (mensajeP.getMatchNumber() != null) {
					miString.append("'" + mensajeP.getMatchNumber() + "'")
							.append(",");
				} else {
					miString.append("' '").append(",");
				}
				if (mensajeP.getExecutedQuantity() != null) {
					miString.append("'" + mensajeP.getExecutedQuantity() + "'")
							.append(",");
				} else {
					miString.append("0").append(",");
				}
				if (mensajeP.getExecutionPrice() != null) {
					miString.append("'" + mensajeP.getExecutionPrice() + "'")
							.append(",");
				} else {
					miString.append("'0.0'").append(",");
				}
				if (mensajeP.getTradeIndicator() != ' ') {
					miString.append("'" + mensajeP.getTradeIndicator() + "'")
							.append(",");
				} else {
					miString.append("' '").append(",");
				}
