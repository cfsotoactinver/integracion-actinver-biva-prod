...
public class DecodificadorDeMensajes {
	...
	final static Logger LOGGER = LoggerFactory
			.getLogger(DecodificadorDeMensajes.class);
	 ...
	 */
	public String getString(byte[] payload, int length)
			throws UnsupportedEncodingException {
	 ...
	 * @throws UnsupportedEncodingException
	 */
	public long getInt(byte[] payload, int length)
			throws UnsupportedEncodingException {
		long value = 0L;
		try {
	 ...
	 * @throws UnsupportedEncodingException
	 */
	public String getDouble(byte[] payload, int length)
	 ...
	 * @throws IOException
	 */
	public String parse(byte[] arr, int length, ArrayList<Object> fieldArray)
			throws IOException {
		String value = "";
	...
	public List<Message> messageIn(byte[] payload) throws IOException {
		...
		long longitudDelPrimerMensaje = getInt(
				Arrays.copyOfRange(payload, 20, 22), 2);
		Message message = null;
		// LOGGER.debug("SESION::" + sesion + " SECUENCIA::" + secuencia +
		// " NUMERO DE MENSAJES::" + numeroDeMensajes + " LONGITUD::" +
		// longitudDelPrimerMensaje + " LONGITUD TOTAL DEL PAYLOAD " +
		// payload.length);
		...
		for (long contador = 0; contador < numeroDeMensajes; contador = contador + 1) {
			...
			String tipoDeMensaje = this.getString(
					Arrays.copyOfRange(payload, 0, 1), 1);
			messageArray.add(tipoDeMensaje);
			messagePointer = 1;
			ArrayList<Object> fieldsArray = null;
			try {
				fieldsArray = this.configuracion.getFields(tipoDeMensaje);
				// LOGGER.debug("Longitud de la lista de campos " +
				// fieldsArray.size());
			} catch (Exception exception) {
				secuencia = secuencia + 1;
				LOGGER.error(
						"Error al obtener la configuración del mensaje con tipo "
								+ tipoDeMensaje, exception);
			}
			LOGGER.debug("Decodificacion" + fieldsArray);
			if (fieldsArray != null) {
				int messagePointerFinal = 0;
				for (int i = 1; i < fieldsArray.size(); i++) {
					ArrayList<Object> fieldArray = this.configuracion
							.getFormat((String) fieldsArray.get(i));
					messagePointerFinal = (messagePointer + ((Integer) fieldArray
							.get(1)));
					// LOGGER.debug("INITIAL::" + messagePointer + "::FINAL::" +
					// messagePointerFinal + ":: RANGE::" + (messagePointerFinal
					// - messagePointer));
					messageArray.add(parse(Arrays.copyOfRange(payload,
							messagePointer, messagePointerFinal),
							messagePointerFinal - messagePointer, fieldArray));
					messagePointer = messagePointer
							+ ((Integer) fieldArray.get(1));
				}
				...
				if (message.getType().charValue() == 'T') {
					this.timeUtils.setLastTimeStamp(((T) message)
							.getTimestamp().longValue());
				}
				message.setFechaHoraRecepcion(this.simpleDateFormat
						.format(new Date()));
				message.setSession(sesion);
				message.setFechaHoraEmision(this.timeUtils
						.getTimeFromMidnightPlusSecondsAndNanoSeconds(
								this.timeUtils.getLastTimeStamp(), message
										.getTimestamp().longValue()));
				
				mensajes.add(message);
			} else {
				LOGGER.error("No se encontraron campos definidos en el archivo YAML para el tipo de mensaje: "
						+ tipoDeMensaje);
			}
		}
		return mensajes;
